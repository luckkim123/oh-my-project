...
**판정 (2026-08-14, 사용자 위임): E 는 지금 만들지 않는다 — 조건부 보류.** 공백은 실재하지만
스키마 필드만 먼저 넣는 형태로는 안 된다. 착수 조건은 **`rules.json` 필드 · `omp-codify` 등록
게이트 · `omp-audit` 축 · `omp-brief` 열거를 한 변경 안에서** 내는 것이고, 그게 부담스러우면
안 만드는 쪽이 낫다 — 위 `external_sources` 시체가 반쪽 구현의 결과다. 같은 이유로 C1
(`placement[]`)도 기각했고 대신 기존 `role` 산문 + PostToolUse 신호로 덮었다(omp 0.11.0).
D9~D11 은 그동안 사람이 Notion 을 직접 보는 것으로 남는다.
