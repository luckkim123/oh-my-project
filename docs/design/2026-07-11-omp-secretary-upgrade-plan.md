...
- **para preset 확장**: para.md는 이미 `Kanban.md`·`Dashboard.md`·`daily_notes/`를 감지 신호(para.md:22-24)와 운영 노트 화이트리스트(para.md:149)로 알고 있다 — 단 전부 *배치* 대상으로만. 확장의 실제 내용은 이 기지(旣知) 표면을 `secretary.sources[]` codify 후보로 *제안*하도록 연결하는 것 + README 상태표류 관례 감지 추가다(자동 등록 아님 — "ML 출력은 사람의 해석으로 검토될 제안으로 프레이밍", [arXiv:2307.16481](https://arxiv.org/pdf/2307.16481)).
...
| R2 | para preset 확장 — 기지 표면(Kanban·daily_notes, para.md:22-24)을 secretary.sources codify 후보 제안으로 연결 + 상태표 관례 감지 추가 | preset prose 확장, 게이트 경유 |
