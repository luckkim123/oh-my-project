...
- Did you run `hooks/omp_secretary.scan_stale(root, now)` over `.omp/secretary/` and report `stale_task`/`stale_blocker`/`brief_drift`/`conflict_copy`/`axis_dormant` findings as warn (never blocking overall PASS, never fixed directly — handed off to omp-review/omp-brief)?
