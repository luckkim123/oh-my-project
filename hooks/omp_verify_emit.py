...
def build_reminder(reason: str) -> str:
    return (
        ...
        ".hq/config/project/STRUCTURE.md·rules.json(+경로 적힌 경우 DATASETS.md) 갱신은 "
        "이 작업의 일부 — 옛 경로가 인덱스에 남지 않게 다음 작업으로 넘어가기 전에 동기화한다.\n"
        "- .hq/config/project/rules.json·manifest.json을 손댔으면 스키마 정합을 확인."
    )
...
def _throttle_path(root):
    """The anchor-gated throttle-state path — same resolver for read and
    write now that stage 2 makes them one computation (omp_paths._resolve)."""
...
def record_emit(root, reason: str, now: float = None) -> None:
    ...
    state[key] = now
    try:
        atomic_write_json(_throttle_path(root), state)
    except Exception:
...
def detect(tool_name: str, tool_input: dict, root=None, rules: dict = None) -> str:
    ...
    if tool_name in WRITE_TOOLS:
        ...
        if is_inside_store(fp):
            return "omp SSOT 파일이 수정됨(.hq/ 또는 .omp/)."
