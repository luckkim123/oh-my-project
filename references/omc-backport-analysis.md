...
> "a management loop that keeps updating one living `.hq/`." When porting OMC patterns, reinterpret *stage = management-loop phase*,
...
| `skill-bodies/autopilot/SKILL.md` | brief→completion stage orchestration + gate skeleton → **omp-pilot** (if no store exists, absorb init → codify→organize→dataset→doc) |
...
| T5 | ralplan sequential consensus | `skills/omp-codify/SKILL.md` enforces sequencing on rule changes (parallel-prohibited) — because rules trigger file moves, concurrent changes are forbidden. The consensus output = a separate file inside `.hq/` (T7 below) |
| T7 | shared_memory handoff | inter-stage transfer = `.hq/` files by **default** (`config/project/`: learned.md · consensus notes; `community/`: wiki/), MCP an optional mirror (degrades to .md when absent) |
...
| T8b | autopilot wiring | in `skills/omp-pilot/SKILL.md` `<Steps>`, inserts an "absorb init when no store exists" branch + a "codify --consensus on large rule changes" branch — the engine actually fires on the pilot path (preventing dead code) |
| T10 | wiki accumulation (the light channel) | `<project>/.hq/community/wiki/*.md` + deterministic grep by **default**, with `wiki_query(category)` an abstract function (a future MCP swap point). Patterns/decisions auto-appended (no approval needed) — the light channel of "general→specialized" evolution |
...
| T15 | state path | SSOT = fixed at `<project>/.hq/` (no unverified segments like `.omp/specs` or `sessions/{sid}`). The 30s state-MCP trap is *a future-proofing note only* |
...
| T18 | OMC `REFERENCE.md` artifact-first handoff + descriptor (kind/path/contentHash/producer) | `.hq/work/project/{scans,versions,plans,audits,tmp}` is this spirit — large intermediates (scan inventories, move plans, audit reports) are left as files and referenced by path rather than pasted into the control plane. Separates the SSOT (top of .hq) from the work layer (output-layout.md) |
...
| **actual state MCP calls** | overkill for the management-loop philosophy. `.hq/` (.md) does surviving-compaction and inter-session handoff better. The 30s trap is documentation-only |
...
the content audit axis `check_content_rule`, dead-link `find_dead_links`, `.hq/community/CONVENTIONS.md`, the specificity content item)
...
repeatedly re-checks one living `.hq/` with rules.json regular expressions**), whereas the siblings oms/omd/omx are *generation
