...
| T2 | critic 4기법 | `agents/auditor.md` 의 규칙 준수 판정 *안에* pre-commitment·assumption(V/R/F)·pre-mortem(어떤 재배치가 데이터를 깰까)·self-audit(LOW confidence→사람 확인 목록) 삽입 |
| T4 | ralplan RALPLAN-DR + architect steelman | `agents/rule-architect.md` 에 `<Consensus_RALPLAN_DR_Protocol>`(Principles/Drivers/Options≥2/steelman/tradeoff/ADR/Short·Deliberate). 큰 rules.json 변경·learn 승격이 파일 이동을 유발할 때만 Deliberate 발동 |
