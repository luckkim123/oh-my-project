...
- **wikilink integrity**: post notes with broken `[[link]]` (dead links) (`hq post`, light channel) as health hints; omp-organize's dry-run plan carries an inbound-`[[link]]` count per move candidate (Release 2) — broken-link risk is visible before approval.
