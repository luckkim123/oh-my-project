...
- Stage outputs and progress state are fixed at **`<project>/.hq/`** (a verified real path — `community/`: `PROJECT.md`/`NAMING.md`/`wiki/`; `config/project/`: `STRUCTURE.md`/`DATASETS.md`/`rules.json`/`manifest.json`/`learned.md`; unverified sub-segments like `.omp/state`·`sessions/{sid}` are not hard-coded). When codify/dataset changes a .json, regenerate the paired .md in the same pass to keep drift at 0.
