...
4. **dataset-curator에게 단일 위임** — `Task(subagent_type="oh-my-project:dataset-curator", ...)`:
   ...
   - **지시**: `.omp/manifest.json`과 `.omp/DATASETS.md` **두 파일만** 쓴다. SHA-256은 stdlib `hashlib`로 raw 바이트 스트리밍(결정적, lowercase 64-hex), 너무 크면 정직한 `UNHASHED`. `rows`는 실제로 셌을 때만, `lineage`는 실재하는 스크립트·원본 증거가 있을 때만, `split.group`으로 형제 split 묶기. 데이터 파일 `mv`/`cp`/`rm`/push **절대 금지**. DVC/git-lfs 감지 시 `managed_by_external` + 메타만 미러. 스키마 검증 후 쓰기, `datasets[]`는 `id`로 정렬, 모호하면 발명 말고 surface, self-approve 금지("registered — ready for audit" 핸드오프).
