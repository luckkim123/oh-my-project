...
description: |
  ...
  `.hq/community/PROJECT.md`·`STRUCTURE.md`·`NAMING.md`·`DATASETS.md`, grounded in the project-scanner inventory. It absorbs the
  "read the codebase and produce professional documentation" spirit of readme-project, but omp centers on the `.hq/`
...
The targets are the 4 human documents — `community/`: `PROJECT.md` / `NAMING.md`; `config/project/`: `STRUCTURE.md` / `DATASETS.md`.
...
spirit, except the output is not the project itself but the *knowledge about the project* inside `<project>/.hq/`.
...
<Use_When>
- The store already exists (= init done) and you need to newly write or refresh the human docs
...
<Do_Not_Use_When>
- When no omp store exists yet → `omp-init` first (bootstrap already lays down a *draft* of PROJECT/STRUCTURE/NAMING)
...
- When the goal is a deployable README at the repo root, unrelated to `.hq/` knowledge → the user-scope `readme-project` skill
  (omp centers on .hq/ docs, it is not a root-README generator. That said, if the request is to draft a README based on
  PROJECT.md, omp-doc can assist with that conversion — but the output's SSOT is .hq/.)
</Do_Not_Use_When>
...
- ⚠️ **Read-only dispatch.** The project-scanner this stage calls is a read-only agent
  (`disallowedTools: [Write, Edit, NotebookEdit]`). It only retrieves the inventory; *writing* the human docs (`community/PROJECT.md`/`NAMING.md`, `config/project/STRUCTURE.md`/`DATASETS.md`) is
...
- ⚠️ **Managed-hash check immediately before a wholesale rewrite (§2.6 governance improvement).** Right before overwriting
  a target .md wholesale, compare its on-disk content hash against the latest snapshot recorded in `.hq/work/project/versions/`
...
1. **Determine target documents.** Figure out which document(s) the user wants — a refresh of all 4
   (`PROJECT.md`/`STRUCTURE.md`/`NAMING.md`/`DATASETS.md`), or a specific one. First confirm whether the store actually
   exists (neither `.hq/.anchor` nor a legacy `.omp/` → guide to `omp-init` and stop). Path convention is governed by `references/output-layout.md` as SSOT.
...
5. **Write/update the documents (controller writes the .md).** Write the .md grounded in the project-scanner inventory:
   - `PROJECT.md` — single-screen overview (what project, core purpose, entry points). Absorb readme-project's
     hero/value-prop spirit into .hq/: scanability > completeness.
...
6. **Task dispatch (inventory collection — always the core of the final step).** The measured inventory that *grounds*
   ...
   Task(
     ...
     description="Inventory project tree for .hq human docs",
     prompt="""
     ...
     - Mismatches between the existing .hq/config/project/rules.json·manifest.json and reality (folders in the docs but absent, and vice versa)
     ...
     ⚠️ read-only — do not create or move files (writing the human docs under `.hq/community/`·`.hq/config/project/` is done by the omp-doc controller).
