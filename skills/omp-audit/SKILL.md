...
name: omp-audit
description: |
  A summative gate that mechanically checks a project folder against .hq/config/project/rules.json and manifest.json —
...
<Purpose>
Mechanically checks whether a project folder actually honors, on disk, the rules declared in `.hq/config/project/rules.json` and `.hq/config/project/manifest.json`. Delegates as a single handoff to `oh-my-project:auditor` (opus, read-only) to judge structure violations, naming violations, dataset checksum drift, split leakage, and lineage/orphan integrity as per-item PASS/FAIL plus evidence. This is the "CI" of code — it judges purely on objective evidence (file:line, expected vs. actual hash, grep results) and offers no critique, advice, or improvement suggestions.
...
- If there is no `.hq/` yet (not init'd) → run `omp-init` first. Without rules.json there is no basis for inspection.
...
- **Bind the verdict to a snapshot identifier** — every PASS/FAIL is bound to the snapshot identifier of what was actually verified that round (content hash of `.hq/config/project/rules.json` / `.hq/config/project/manifest.json` + SHA-256/mtime of the inspected dataset files + the set of violation IDs handled). Because the identifier changes when `omp-organize` moves files or `omp-codify` changes rules, a stale PASS is not reused across an audit→organize→re-audit multi-round.
...
- All paths are relative to the project root (`pathlib`) — no absolute path or `~` hardcoding. The `rules.json.ignore[]` globs (`.git/**`, `node_modules/**`, `.hq/**`, etc.) are excluded from inspection.
</Execution_Policy>

<Steps>
1. **Confirm target and SSOT**: confirm the project root and the existence of `.hq/config/project/rules.json` and `.hq/config/project/manifest.json`.
   - If `.hq/` is absent, audit is impossible → report "`omp-init` needed" to the caller and stop (no inspection basis).
...
2. **Fix the inspection axes**: state which axes this round covers — structure violations (`rules.json.structure.directories[].enforced`), naming violations (`naming.patterns[]` Python regex + severity), content-convention violations (`content_conventions[]` applies_to × check.pattern/expect/scope, enforced — error→FAIL), wikilink integrity (vault `[[link]]` resolution, info — dead link = health hint, not a FAIL), dataset checksum drift (`manifest.datasets[].sha256` SHA-256 recompute), split leakage (same hash/path within a `split.group`), lineage/orphan integrity, specificity consistency (info), **Docker anti-pattern** (`.hq/config/project/env/` + root `Dockerfile`/compose files, via `hooks/omp_docker_audit.check_dockerfile`/`check_compose`; rule-ids carried as data: `DL3007`/`secret-in-env`/`compose-version`; **warn-default — GUI/sim containers legitimately break these rules, so docker findings do NOT block an overall PASS**; a project may opt out a rule-id via `rules.json` severity-override). **`docker_images[]` are NOT checksum-drift-checked** — an image has no in-tree path and a remote digest cannot be re-hashed with `hashlib` (see auditor.md), and **secretary hygiene** (`.hq/config/project/secretary/` — via `hooks/omp_secretary.scan_stale(root, now)`, the canonical algorithm; finding kinds `stale_task`/`stale_blocker`/`brief_drift`/`conflict_copy`/`axis_dormant`; **warn-default — never blocks an overall PASS**, same idiom as the Docker axis; audit only reports, it does not fix — stale findings hand off to `omp-review`'s BuJo migration, `brief_drift` hands off to `omp-brief`'s regeneration), and **governance hygiene** (`hooks/omp_content_audit.scan_structure_drift(root, rules)` — flags `rules.json.structure.directories[].path` entries and backtick-quoted paths in `STRUCTURE.md`/`DATASETS.md` that no longer exist on disk; `hooks/omp_content_audit.lint_wiki(root, now)` — 8-kind lint over `.hq/community/wiki/*.md` and `.hq/config/project/learned.md`: `orphan` (no `[[backlink]]` from another note), `stale` (mtime > 30d), `oversized` (> 50KB), `open_item` (an unchecked `- [ ]` commitment in a wiki note — the resurfacing channel for what the notes promised to do later, per `references/output-layout.md`; **no age gate and no prose scan** — file mtime answers "was the page edited" not "how long has this item sat", and a prose-marker scan measured 3 false positives in 7 hits because a heading declaring an item *resolved* carries the same words as the item), `broken-ref` (documented alias for the existing `find_dead_links` wikilink check), `stuck_candidate` (a `learned.md` OBS block with `evidence_count < 3` and `first_seen` > 30d old — never promotable but never re-evaluated either), `ready_to_promote` (a candidate OBS block that reached `evidence_count >= 3` with `counter_examples == 0` and `user_overridden` false, per learning-protocol §3 — ripe for `omp-learn` promotion, otherwise invisible to enumeration), `contradiction` (two candidate OBS blocks targeting the same `applies_to` glob with conflicting `path_constraint`); **warn-default — same idiom as Docker/secretary, findings do NOT block an overall PASS** — these are second-brain hygiene signals for `omp-learn`/`omp-codify` to triage, not enforceable violations), and **store-layer integrity** (`rules.json.layers` via `hooks/omp_content_audit.scan_layers(root, rules)` — every `.hq/` anchor at or below root; finding kinds `layer_unknown` (a top-level entry under the store root that is not a declared layer), `layer_tracked` (a layer declared tracked that git actually ignores), `layer_ignored` (a layer declared ignored that git does not), `anchor_parse` (`.anchor` is not exactly one `id: <slug>` line); **error-default — findings DO block an overall PASS**, unlike the hygiene axes: an over-broad ignore rule over `community/` loses the human record on the next clone and nothing errors, so this is data loss rather than untidiness. **Per-file layer assignment is NOT re-derived here** — `claudebase`'s `migrate-om-store.sh` owns that table and a second copy would drift invisibly. Absent `rules.json.layers` the axis returns nothing, because a project that never adopted the unified store is not in violation of it), and **code-graph registry** (`rules.json.code_graphs.indexes[]` via `hooks/omp_graph_audit.scan_graphs(root, rules)` — finding kinds `graph_missing` (registered but absent on disk), `graph_stale` (CRG's `Built at commit` ≠ HEAD, or graphify's `needs_update` marker), `graph_coverage_mismatch` (a language in `covers` that the index does not actually hold); **warn-default — findings do NOT block an overall PASS**, same idiom as the Docker/secretary/governance axes. The axis exists because a graph tool answers "is there a graph?" but never "does it cover what you are about to ask it": measured on one vault, a CRG index of 21,865 nodes held 101 files of vendored plugin JS while the repo tracked 2,277 files including 828 notes, and nothing reported the gap. **omp never rebuilds an index** — a finding quotes the entry's own `refresh` command for a human to run, and **`tokensave` is never executed at all** because its CLI re-installs agent integration as a side effect of read-only-looking commands). Even if the calling context requests only one axis, the dataset axis and the structure/naming axes run independently of each other — a FAIL on one axis does not cause another axis's check to be skipped.
3. **Load reference cards (delegation input)**: gather the SSOT and card paths to hand to the auditor —
   - `.hq/config/project/rules.json` + `references/schemas/rules.schema.json` (rule schema SSOT: required fields `omp_version/project/specificity/structure/naming`)
   - `.hq/config/project/manifest.json` + `references/schemas/manifest.schema.json` (inventory schema SSOT: required `omp_version/generated/datasets`, sha256 `^[a-f0-9]{64}$|^UNHASHED$`)
   - `.hq/config/project/STRUCTURE.md` / `.hq/community/NAMING.md` / `.hq/config/project/DATASETS.md` (human-facing .md — paired with the machine .json, for reference)
   - `references/output-layout.md` (`.hq/` path conventions and "Two learning channels" SSOT)
...
6. **Attach snapshot identifier + record the report**: bind the verdict to the verified-target snapshot identifier to block stale-PASS reuse after organize/codify. Then record the per-item PASS/FAIL + evidence + snapshot identifier to `.hq/work/project/audits/audit-{YYYY-MM-DD-HHMM}.json` to leave a drift history (time-series comparison). ⚠️ **This record is written by this skill (the controller), not by the read-only auditor** — the auditor only produces output (text), and the work/audits record is the calling skill's job (preserving the auditor's read-only invariant and the detection≠execution separation). After recording, retention-prune `.hq/work/project/audits/`: keep only the latest N=10 and prune older reports via trash (no permanent `rm`), with a one-line "pruned X old audits" report — this skill that wrote the record trims its own subfolder. `references/output-layout.md` work layer.
7. **Task delegation (single, read-only)**: bundle the above input and delegate to the auditor as a single handoff. Because it is a read-only gate, use a **single dispatch** for the independence of the automated check (do not split the inspection items across multiple agents — one auditor views all axes deterministically):
   ...
   Task(
     ...
     SSOT: .hq/config/project/rules.json, .hq/config/project/manifest.json
     ...
     For dataset checksums, do not trust the manifest-recorded value; judge only by hashlib SHA-256 recompute.
       ...
       - Docker anti-pattern (run hooks/omp_docker_audit.check_dockerfile / check_compose over .hq/config/project/env/ and root Dockerfile/compose; rule-ids DL3007/secret-in-env/compose-version; warn-default — findings do NOT block overall PASS; project may override severity per rule-id via rules.json; docker_images[] are NOT drift-checked — no in-tree path, remote digest un-rehashable)
       - Secretary hygiene (run hooks/omp_secretary.scan_stale(root, now) over .hq/config/project/secretary/; finding kinds stale_task/stale_blocker/brief_drift/conflict_copy/axis_dormant; warn-default — findings do NOT block overall PASS; audit only reports, no fix — stale findings hand off to omp-review, brief_drift hands off to omp-brief)
       - Store-layer integrity (run hooks/omp_content_audit.scan_layers(root, rules) for the .hq/ tracked/ignored boundary and undeclared layers: layer_unknown/layer_tracked/layer_ignored/anchor_parse; error-default — findings DO block overall PASS; inert when rules.json has no layers block)
       - Governance hygiene (run hooks/omp_content_audit.scan_structure_drift(root, rules) for structure.directories[]/STRUCTURE.md/DATASETS.md paths gone missing on disk, and hooks/omp_content_audit.lint_wiki(root, now) for the 8-kind wiki/learned.md lint: orphan/stale/oversized/open_item/broken-ref/stuck_candidate/ready_to_promote/contradiction; warn-default — findings do NOT block overall PASS; audit only reports — triage is omp-learn/omp-codify's domain)
       - Code-graph registry (run hooks/omp_graph_audit.scan_graphs(root, rules) over rules.json code_graphs.indexes[]; finding kinds graph_missing/graph_stale/graph_coverage_mismatch; warn-default — findings do NOT block overall PASS; audit only reports and NEVER rebuilds an index — each finding carries the entry's own `refresh` command for a human to run. ⚠️ tokensave is never executed: its CLI re-installs agent integration as a side effect of read-only-looking commands, so a tokensave entry is checked for presence only and its coverage is undeclarable. The axis is silent when code_graphs is absent, which is the common case.)
...
<Output>
- **Per-item result table**: schema validity / structure violations / naming violations (error, warn, info) / content conventions (error, warn, info) / wikilink integrity (dead link info) / dataset checksum drift / split leakage / lineage integrity / manifest path existence (orphan, unregistered) / specificity consistency / Docker anti-pattern (warn) / secretary hygiene (warn: stale_task, stale_blocker, brief_drift, conflict_copy, axis_dormant) / governance hygiene (warn: structure_drift, orphan, stale, oversized, broken-ref, stuck_candidate, ready_to_promote, contradiction) / code-graph registry (warn: graph_missing, graph_stale, graph_coverage_mismatch) — each PASS / FAIL / WARN + remarks.
