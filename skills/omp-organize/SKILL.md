...
7. **Post-verification + recording**: after moving, confirm with `omp-audit` (or auditor re-detection) that the violations were actually resolved (target: 0 residual violations). Lightweight patterns surfaced during organizing are posted (`hq post`); heavy observations worth turning into rules are written as candidates to `.hq/config/project/learned.md` and handed to `omp-learn`.
...
- `.hq/community/posts/` post entries (`hq post`) / `.hq/config/project/learned.md` promotion candidates (→ point to omp-learn)
