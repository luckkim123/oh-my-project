...
- Light decisions/patterns (e.g., "why this rule was added this time") may be posted (`hq post --topic decision`) (no approval needed, recoverable via `hq query --ascend` next session). Only heavy rule changes go through the gate (design §4, 2 channels).
</Execution_Policy>
