...
<Purpose>
Generates and manages Dockerfile/docker-compose canonical files into `.hq/config/project/env/` — the single
authoritative location for environment assets within this project's `.hq/` store
...
<Use_When>
- The store exists (omp-init done) and you need to create or update a Dockerfile or
...
<Do_Not_Use_When>
- No omp store exists yet → **omp-init first**.
- You only want to define docker_naming rules (what images/containers should be called) →
  **omp-codify** (writes docker_naming block into `.hq/config/project/rules.json` + NAMING.md).
- You only want to register image metadata (SHA digest, registry, pull date, lineage) →
  **omp-dataset** (writes docker_images block into `.hq/config/project/manifest.json` + DATASETS.md).
...
<Execution_Policy>
- ⚠️ **Canonical files live in `.hq/config/project/env/` ONLY.** Never write Dockerfile or compose files
  outside `.hq/config/project/env/`. The project root (or any other location) receives only a symlink or a
  sync view — the in-place invariant of `.hq/` is preserved (`references/output-layout.md`).
...
- ⚠️ **Stage boundaries are hard.** docker_naming rules (image/container naming patterns,
  tag conventions) are written by omp-codify into `.hq/config/project/rules.json`. docker_images inventory
...
2. **Check docker_naming rules.** Read `.hq/config/project/rules.json` for any existing `docker_naming`
...
3. **Draft assets (dry-run).** Generate the Dockerfile and/or docker-compose draft in memory,
   ...
   diff against any existing `.hq/config/project/env/` files (or "(new file)" if none). This is the dry-run
...
5. **Write to `.hq/config/project/env/`.** Once approved, write the canonical files to `.hq/config/project/env/`
   (e.g., `.hq/config/project/env/Dockerfile`, `.hq/config/project/env/docker-compose.yml`). If a root-level symlink or
   sync view is needed, create it pointing to `.hq/config/project/env/<file>` — never copy the content
   outside `.hq/config/project/env/`.
6. **Disk verify.** Confirm presence with `test -f .hq/config/project/env/<file>` for each written file.
   If a compose file was written, run `docker compose -f .hq/config/project/env/docker-compose.yml config`
...
<Output>
Canonical Dockerfile/docker-compose files written to `.hq/config/project/env/` (single authoritative location,
